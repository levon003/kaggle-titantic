...
df <- merge(df, new_cols, by="PassengerId")
```
...
```{r}
toWrite <- df[,c("PassengerId", "fSex", "fEmbarked", "Pclass", "Age", "SibSp", "Parch", "Fare", "Survived", "name_title", "name_word_length", "name_char_length", "cabin_first_letter", "ticket_category")]
...
```{r}
md <- glm(Survived ~ fSex + fEmbarked + Pclass + Age + SibSp + Parch + Fare + name_title + name_word_length:name_char_length + cabin_first_letter + ticket_category, family="binomial", data=train)
summary(md)
```
